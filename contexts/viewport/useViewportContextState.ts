...
const useViewportContextState = (): ViewportContextState => {
  ...
  useEffect(() => {
    ...
    const updateTouch = (): void => setIsTouchDevice(coarsePointer.matches);
    const updateMobile = (): void => setIsMobile(mobileWidth.matches);

    updateTouch();
    updateMobile();

    coarsePointer.addEventListener("change", updateTouch);
    mobileWidth.addEventListener("change", updateMobile);

    return () => {
      coarsePointer.removeEventListener("change", updateTouch);
      mobileWidth.removeEventListener("change", updateMobile);
    };
  }, []);

  return { fullscreenElement, isMobile, isTouchDevice, toggleFullscreen };
};
