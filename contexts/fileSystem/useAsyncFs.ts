...
const useAsyncFs = (): AsyncFSModule => {
  ...
  useEffect(() => {
    if (!fs) {
      ...
      const checkFsVersion = async (): Promise<void> => {
        const FS_VERSION_KEY = "pyhdra-os-fs-version";
        const currentVersion = PACKAGE_DATA.version;
        let storedVersion = "";
        try {
          storedVersion = localStorage.getItem(FS_VERSION_KEY) || "";
        } catch {
        ...
        if (storedVersion !== currentVersion) {
          ...
          try {
            localStorage.setItem(FS_VERSION_KEY, currentVersion);
          } catch {
            // Ignore storage errors
          }
        }
      ...
      checkFsVersion().finally(() => supportsIndexedDB().then(setupFs));
    }
  }, [fs]);
