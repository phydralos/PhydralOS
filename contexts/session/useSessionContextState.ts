...
const useSessionContextState = (): SessionContextState => {
  const { deletePath, readdir, readFile, rootFs, lstat } =
    useFileSystem();
  const [sessionLoaded, setSessionLoaded] = useState(false);
  const [isLocked, setIsLocked] = useState(true);
  const [authUser, setAuthUser] = useState<AuthUser | null>(null); // eslint-disable-line unicorn/no-null
  const [userEmail, setUserEmail] = useState("user@pyhdral.com");
  ...
  const haltSessionRef = useRef(false);
  const setHaltSession = useCallback((value: boolean) => {
    haltSessionRef.current = value;
  }, []);
  ...
  useEffect(() => {
    // Session saving disabled — nothing persists across reboots
  }, []);
    ...
    if (!initializedSession.current && rootFs) {
      const initSession = async (): Promise<void> => {
        ...
        try {
          ...
          setWallpaper(DEFAULT_WALLPAPER, session.wallpaperFit);
          if (
            session.sortOrders &&
          ...
          if (
            ...
            setWindowStates(session.windowStates);
                    ...
                    if (session.widgetsEnabled !== undefined) {
            setWidgetsEnabled(session.widgetsEnabled);
          }
