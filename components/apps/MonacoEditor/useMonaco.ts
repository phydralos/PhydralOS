...
const useMonaco = ({
  ...
  useEffect(() => {
    if (!monaco) {
      // Ensure define exists with amd property before shareGlobal intercepts it
      const windowWithDefine = window as unknown as {
        define?: ((...args: unknown[]) => void) & { amd?: unknown };
      };
      if (typeof windowWithDefine.define !== "function") {
        // eslint-disable-next-line @typescript-eslint/no-empty-function
        windowWithDefine.define = function define() {};
        windowWithDefine.define.amd = {};
      }

      shareGlobal("define", "MonacoEditor", 2.5 * MILLISECONDS_IN_SECOND);

      const monacoLoader =
        (loader as unknown as { default?: typeof loader }).default || loader;

      monacoLoader.config(config);
      monacoLoader
        .init()
        .then((monacoInstance: typeof Monaco) => setMonaco(monacoInstance));
    }
  }, [monaco]);
