    ...
    case SHORTCUT_EXTENSION:
      {
        const handleShortcut = ({
          ...
          } else if (pid === "LoadedApp" && icon) {
            const loadedAppIcon = icon;
            fs.readFile(loadedAppIcon, (readErr, iconData) => {
              callback({
                comment,
                icon:
                  !readErr && iconData
                    ? bufferToUrl(iconData, getMimeType(loadedAppIcon))
                    : processDirectory.LoadedApp?.icon || loadedAppIcon,
                pid,
                subIcons,
                url,
              });
