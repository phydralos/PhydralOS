...
import { installAppFromZip, isAppPackage } from "utils/appInstaller";
  ...
  const newPath = useCallback(
    async (
      ...
      if (uniqueName && !uniqueName.includes("/")) {
        ...
        if (
          buffer &&
          ZIP_EXTENSIONS.has(getExtension(uniqueName)) &&
          directory === DESKTOP_PATH &&
          (await isAppPackage(buffer))
        ) {
          try {
            ...
            if (result.success) {
              ...
              try {
                await deletePath(join(directory, uniqueName));
                updateFolder(directory, undefined, uniqueName);
              } catch {
                // Ignore deletion error - file may already be removed
              }
            }
          } catch (error) {
